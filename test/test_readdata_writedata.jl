...
@testset verbose=true "readdata/writedata roundtrips" begin
    ...
    @testset "nested arrays" begin
        ...
        vom = nestedview(rand(3, 4, 5), Val(2))
    ...
    @testset "encoded arrays" begin
        ...
        vosv = VectorOfSimilarVectors([rand(Int32(-5):Int32(5), 20) for _ in 1:10])
    ...
    @testset "waveforms" begin
        ...
        wfs = ArrayOfRDWaveforms((fill(trng, 20), nestedview(rand(UInt16, 50, 20)*u"m")))
        r = _roundtrip(wfs)
        ...
        codec = VarlenDiffArrayCodec()
        signal = VectorOfSimilarVectors([rand(Int32(-5):Int32(5), 50) for _ in 1:20])
